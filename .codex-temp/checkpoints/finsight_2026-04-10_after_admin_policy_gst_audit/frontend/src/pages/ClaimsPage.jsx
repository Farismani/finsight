                ...
                {claims.map((claim) => (
                  <tr key={claim.id} className="text-sm text-slate-200">
                    ...
                    <td className="px-4 py-4">
                      ...
                      {claim.admin_status ? (
                        <div className="mt-2 max-w-xs space-y-0.5 text-xs leading-5 text-slate-400">
                          <p className="font-medium text-slate-300">{formatAdminAction(claim.admin_status)}</p>
                          <p>Reason: {claim.admin_reason || 'No reason provided'}</p>
                          {formatAuditDate(claim.admin_timestamp) ? (
                            <p>Time: {formatAuditDate(claim.admin_timestamp)}</p>
                          ) : null}
                        </div>
                      ) : null}
                    </td>
